...
- 非同期処理でセットステート関数の実行を遅延しないといけないケースが存在するので区別しない
- stateの変更に対して常に参照透過性(x = func(y)のyが同じ値であれば常にxが同じ値になるので処理結果でそのま式を展開できる性質)を持たせる

３個目の項目は厳密には参照を渡した場合副作用が発生する可能性がありますが、セットステート関数を用いて値を変更し続ける限り、state自体に直接副作用は発生しなくなります。また、Reactは代入する変数が参照の場合は毎回参照剥がしを行う事を推奨しています。

これはパフォーマンスの話の時に説明しますが、Reactはpureコンポーネントでレンダリング最適化のチューニングをする場合Reconcilationを行う際に二階層目以上のプロパティの参照比較を行いません。つまり、propsやstateに紐づくメンバ変数の参照のプロパティが変わっていたとしても、参照アドレスが変わっていない場合はshallow comaprisonで再レンダリング条件に引っかからず、これによってレンダリングが走らないバグが起こり得ます。


これを防ぐために、できる限りpropsは参照ではなくプリミティブ型の値を用いるようにコンポーネントを設計し、参照を用いる場合はセットステート関数で`{...params}`、`[...params]`などのスプレッドオペレーターでパラメータを展開して渡しましょう。これにより、paramsの要素の中に参照がいない限り、
