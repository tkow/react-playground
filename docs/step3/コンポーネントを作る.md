...
その際に再レンダリングされるコンポーネントの関係を図示します。以下の図では赤いノードでパラメータが差分検知された場合にReconcilationの過程でレンダリング対象になるノードを青で表し、Pureコンポーネントのshallow comparisonの結果再レンダリング対象にならなかったコンポーネントを黄緑、Reconcilation対象にならなかったノードを白で表しています。
